                    ...
                    <div class="row">
                        ...
                        if($prod_count==0) {
                            ...
                            if(isset($_GET['page'])){
                             ...
                             $offset = ($page - 1) * $limit;
                             $full_page_product = count($final_ecat_ids)-1;
                            for($ii=0;$ii<count($final_ecat_ids);$ii++) {
                                $statement = $pdo->prepare("SELECT * FROM tbl_product WHERE ecat_id=? AND p_is_active=? LIMIT {$offset}, {$limit}");
                                $statement->execute(array($final_ecat_ids[$ii],1));
                                $statement_c = $pdo->prepare("SELECT * FROM tbl_product WHERE ecat_id=? AND p_is_active=?");
                               $statement_c->execute(array($final_ecat_ids[$ii],1));
                                $countEnd = $statement_c->rowCount();
                                $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                                foreach ($result as $row) {
                                    ?>
                        ...
                        <?php
                            ...
                            $result = $pdo->prepare("SELECT * FROM tbl_product");
                            $result->execute();
                            $count = $result->rowCount();
                            //echo $count;
                            if($category=='end-category'){
                                $totalRecords=$countEnd;
                                ...
                                <ul class="pagination justify-content-end">
                                    ...
                                    ?>
                                    <li class="page-item <?php echo $active;?>"><a class="page-link" href="product-category.php?id=<?php echo $id; ?>&type=<?php echo $category;?>&page=<?php echo $i; ?>"><?php echo $i;?></a></li>
                                <?php } } 

                        elseif($category=='top-category'){

                            if($count>0){
                                $totalRecords = $count;
                                
                                $totalPages = ceil(ceil($totalRecords/$limit)/ $full_page_product);
                        ?>
                    <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-end">
                        <?php
